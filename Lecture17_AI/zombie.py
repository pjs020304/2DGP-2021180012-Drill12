...
class Zombie:
    ...
    def more_than_boy(self):
        if self.ball_count >= play_mode.boy.ball_count:
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL
    def less_than_boy(self):
        if self.ball_count < play_mode.boy.ball_count:
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL
    ...
    def run_away_to_boy(self, r=10):
        self.state = 'Walk'
        if self.x<play_mode.boy.x:
            if self.y<play_mode.boy.y:
                self.set_target_location(play_mode.boy.x-300, play_mode.boy.y-300)
            else:
                self.set_target_location(play_mode.boy.x-300, play_mode.boy.y+300)
        else:
            if self.y < play_mode.boy.y:
    ...
    def build_behavior_tree(self):
        ...
        c2 = Condition('공이 더 많은가?', self.more_than_boy)
        a4 = Action('소년한테 접근', self.move_to_boy)
        root = chase_boy = Sequence('소년을 추적', c1, c2, a4)

        c3 = Condition('공이 더 적은가?', self.less_than_boy)
        a5 = Action('소년한테 후퇴', self.run_away_to_boy)
        root = run_away_boy = Sequence('소년한테 후퇴', c1, c3, a5)
